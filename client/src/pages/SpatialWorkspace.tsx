...
export default function SpatialWorkspace() {
  ...
  const selectedSource = typeof selected?.properties.source === "string" ? selected.properties.source : "Live PostGIS geometry";
  const selectedRecordType = typeof selected?.properties.recordType === "string" ? selected.properties.recordType : "Source-backed geometry";
  const iitPatnaContext = isIitPatnaReference(siteQuery) ? IIT_PATNA_OFFICIAL_CONTEXT : null;
  ...
  const osmBuildingsReady = Boolean(import.meta.env.VITE_CESIUM_ION_ACCESS_TOKEN);
  ...
  const nextEvidenceAction = activeEvidenceLevel === 1 ? "Attach height evidence" : activeEvidenceLevel === 2 ? "Attach floor plan / BIM" : "Review vertical ULPIN";
      ...
      <section className="spatial-workspace-main">
        ...
        <div className="spatial-model-layout">
          <section className="spatial-model-stage">
            ...
            <div className="spatial-action-dock" aria-label="Property workflow actions"><span>Live property workflow</span><div><button type="button" onClick={() => issueCommand("focus-site")}><ScanSearch size={13} /> Focus source</button><button type="button" onClick={() => setLocation(`/?workspace=Data%20ingestion&site=${encodeURIComponent(siteQuery)}`)}><Database size={13} /> {nextEvidenceAction}</button><button type="button" onClick={() => setLocation(selected ? `/?editor=${encodeURIComponent(selected.ulpin)}` : "/?workspace=Operator%20access")}><ShieldCheck size={13} /> {selected ? "Authority review" : "Operator access"}</button></div><small>{activeEvidenceLevel === 1 ? "No height, floor, or unit data is inferred." : activeEvidenceLevel === 2 ? "Extrusion is evidence-backed; floor geometry remains locked." : "Floor-plan/BIM evidence is ready for registered vertical ULPIN review."}</small></div>
          ...
          <aside className="spatial-dossier">
            <div className="spatial-dossier-card"><div className="spatial-dossier-title"><div><p>Structure inspector</p><h2>{selected ? selectedName : "Source-backed 3D preview"}</h2></div><button type="button" onClick={() => issueCommand("inspect-footprint")} aria-label="Inspect a building footprint"><ScanSearch size={16} /></button></div><ThreeBuildingPreview feature={previewFeature} /><div className="spatial-source-facts" aria-label="Source-backed building facts"><span><small>Visual context</small><b>Licensed imagery + public 3D context</b></span><span><small>Geometry record</small><b>{selected ? selectedRecordType : "Live PostGIS query"}</b></span><span><small>Evidence source</small><b>{selected ? selectedSource : "Microsoft / OSM provenance"}</b></span><span><small>Selected reference</small><b>{selected ? selectedUlpIn : "Choose a source footprint"}</b></span></div>{placeFacts.data && <div className="spatial-institution-evidence" aria-label="Source-aware place facts"><p>Source-aware place facts</p><div><b>{placeFacts.data.headline}</b><span>{placeFacts.data.availability === "source-backed" ? `${placeFacts.data.combinedSourceFootprintAreaSquareMetres.toLocaleString()} m² combined source-footprint area` : "No verified geometry returned"}</span><small>{placeFacts.data.dataOrigin}</small></div><div className="spatial-institution-lock"><b>Unavailable measurements</b><small>{placeFacts.data.unavailableMeasurements.join(" ")}</small></div></div>}{iitPatnaContext && <div className="spatial-institution-evidence" aria-label="IIT Patna unlinked official building context"><p>Verified institution context</p>{iitPatnaContext.records.map(record => <div key={record.label}><a href={record.sourceUrl} target="_blank" rel="noreferrer">{record.label} ↗</a><span>{record.floors} · {record.builtUpArea}</span><small>{record.linkage}</small></div>)}{iitPatnaContext.lockedRequests.map(record => <div className="spatial-institution-lock" key={record.label}><b>{record.label} · authority gate</b><small>{record.requirement} {record.outcome}</small></div>)}</div>}<div className="spatial-vertical-profile" aria-label="Ordered vertical review profile">{verticalLayers.map(layer => <button type="button" className={`${verticalLayer === layer.id ? "active" : ""} ${layer.id === "surface" ? "live" : "pending"}`} key={layer.id} onClick={() => activateVerticalLayer(layer.id)}><span>{layer.order}</span><b>{layer.shortLabel}</b><i /></button>)}</div><div className="spatial-dossier-state"><span>Level {activeEvidenceLevel} · {activeEvidence.source}</span><strong>{activeEvidenceLevel === 1 ? "Public footprint visual only. Add a verified building-height record to unlock the extruded building." : activeEvidenceLevel === 2 ? "Verified height unlocks the building extrusion. Add an official floor plan or BIM to unlock floor-by-floor geometry." : "Official floor-plan/BIM evidence supports a floor-by-floor model and vertical ULPIN review."}</strong></div><dl><div><dt>Footprint area</dt><dd>{selected ? selectedArea : `${searchResult.data?.totalFootprintAreaSquareMetres.toLocaleString() ?? "0"} m²`}</dd></div><div><dt>3D height</dt><dd>{selected ? selectedHeight : `${searchResult.data?.approvedHeightCount ?? 0} approved`}</dd></div><div><dt>Floor plan / BIM</dt><dd>{hasOfficialFloorPlan ? "Official evidence" : "Not attached"}</dd></div><div><dt>Ownership</dt><dd>{selected?.properties.ownershipLinked ? "Authority-linked" : "No inferred ownership"}</dd></div></dl>{selected && <button className="spatial-correct-button" type="button" onClick={() => setLocation(`/?editor=${encodeURIComponent(selected.ulpin)}`)}>Review source record</button>}</div>
