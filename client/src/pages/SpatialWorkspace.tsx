  ...
  return (
    <main className="spatial-workspace-shell">
      <aside className="spatial-workspace-rail" aria-label="Spatial workspace navigation">
        ...
        <div className="spatial-rail-status"><i /> <span><small>Active field mode</small><strong>Amity University Patna area</strong><em>OSM reference · individual footprints</em></span></div>
      ...
      <section className="spatial-workspace-main">
        ...
        <div className="spatial-model-layout">
          <section className="spatial-model-stage">
            ...
            <div className="spatial-stage-heading"><p>Live 3D model</p><h1>{searchResult.data?.siteLabel ?? "Amity University Patna"}</h1><span><CircleDot size={13} /> 25.6124° N <b>·</b> 85.0548° E <b>·</b> EPSG:4326</span></div>
            ...
            <div className="spatial-selection-chip"><i /> {selected ? `Selected ${selected.ulpin}` : `${searchResult.data?.buildingCount ?? 0} live building layers`} <b>{searchResult.data?.totalFootprintAreaSquareMetres.toLocaleString() ?? "0"} m²</b></div>
          ...
          <aside className="spatial-dossier">
            <div className="spatial-dossier-card"><div className="spatial-dossier-title"><div><p>Property inspector</p><h2>{selected ? selectedName : "Live footprint layers"}</h2></div><button type="button" onClick={() => issueCommand("inspect-footprint")} aria-label="Inspect a building footprint"><ScanSearch size={16} /></button></div><div className="spatial-volume-preview"><i /><i /><i /><i /></div><div className="spatial-dossier-state"><span>{selected ? "Selected source record" : "Select a building on the live map"}</span><strong>{selected ? selectedUlpIn : `${searchResult.data?.buildingCount ?? 0} individual building polygons`}</strong></div><dl><div><dt>Footprint area</dt><dd>{selected ? selectedArea : `${searchResult.data?.totalFootprintAreaSquareMetres.toLocaleString() ?? "0"} m²`}</dd></div><div><dt>3D height</dt><dd>{selected ? selectedHeight : `${searchResult.data?.approvedHeightCount ?? 0} approved`}</dd></div><div><dt>Ownership</dt><dd>{selected?.properties.ownershipLinked ? "Authority-linked" : "No inferred ownership"}</dd></div></dl>{selected && <button className="spatial-correct-button" type="button" onClick={() => setLocation(`/?editor=${encodeURIComponent(selected.ulpin)}`)}>Review source record</button>}</div>
