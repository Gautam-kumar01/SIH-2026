...
export type LayeredAreaSearchResult = {
  ...
  records: Array<{ ulpin: string; name: string; footprintAreaSquareMetres: number; approvedHeightMetres: number | null; parcelReference: string | null; ulpinRecord: string | null }>;
};
  ...
  return {
    ...
    features: result.rows.map(row => ({
      ...
      properties: {
        ...row.properties,
        ulpin: row.ulpin,
        approvedHeightMetres: row.approved_height_metres === null ? null : Number(row.approved_height_metres),
        ...
        ownershipData: row.ownership_record_id === null ? null : {
        ...
        footprintAreaSquareMetres: Math.round(Number(row.area_square_metres) * 100) / 100,
...
export async function searchPostgisLayeredArea(query: string): Promise<LayeredAreaSearchResult> {
  const normalizedQuery = query.trim();
  const searchQuery = /^cimage$/i.test(normalizedQuery) ? "Amity University Patna" : normalizedQuery;
  const result = await getPool().query<{
    ulpin: string;
    name: string | null;
    area_square_metres: string | number;
  ...
  const buildingRecords = records.filter(record => record.footprintAreaSquareMetres > 0);
  return {
    query: normalizedQuery,
    siteLabel: /^cimage$/i.test(normalizedQuery) ? "Amity University Patna reference area" : buildingRecords.find(record => /amity university patna/i.test(record.name))?.name || searchQuery,
    matchedUlpins: buildingRecords.map(record => record.ulpin),
    buildingCount: buildingRecords.length,
    totalFootprintAreaSquareMetres: Math.round(buildingRecords.reduce((sum, record) => sum + record.footprintAreaSquareMetres, 0) * 100) / 100,
    approvedHeightCount: buildingRecords.filter(record => record.approvedHeightMetres !== null).length,
    ownershipLinkCount: buildingRecords.filter(record => record.ownershipRecordId !== null).length,
    records: buildingRecords.map(({ ownershipRecordId: _ownershipRecordId, ...record }) => record),
  };
}

export function isValidFootprintGeometry(value: unknown): value is GeoJsonGeometry {
  if (!value || typeof value !== "object") return false;
  const candidate = value as { type?: unknown; coordinates?: unknown };
  if ((candidate.type !== "Polygon" && candidate.type !== "MultiPolygon") || candidate.coordinates === undefined) return false;
  const hasFiniteCoordinate = (coordinates: unknown): boolean => {
    if (!Array.isArray(coordinates) || coordinates.length === 0) return false;
    if (typeof coordinates[0] === "number") return coordinates.length >= 2 && coordinates.every(value => typeof value === "number" && Number.isFinite(value));
...
export async function upsertPostgisGeoJsonFeatures(featureCollection: unknown) {
  ...
  try {
    ...
    for (const feature of features) {
      ...
      if (typeof ulpinCandidate !== "string" || !ulpinCandidate.trim() || !isValidFootprintGeometry(input.geometry)) continue;
      await client.query(
...
export async function updatePostgisFootprint(update: FootprintUpdate) {
  ...
  try {
    await client.query("BEGIN");
    const current = await client.query<{ id: number; geometry_revision: number }>("SELECT id, geometry_revision FROM property_geometry WHERE ulpin = $1 FOR UPDATE", [update.ulpin]);
    if (!current.rows[0]) throw new Error("The selected footprint was not found in Neon PostGIS");
    const geometryJson = update.geometry ? JSON.stringify(update.geometry) : null;
    if (geometryJson) {
      const validity = await client.query<{ valid: boolean }>("SELECT ST_IsValid(ST_SetSRID(ST_GeomFromGeoJSON($1), 4326)) AS valid", [geometryJson]);
      if (!validity.rows[0]?.valid) throw new Error("The proposed footprint geometry is invalid in PostGIS");
    }
    ...
    `, [update.ulpin, geometryJson, update.approvedHeightMetres ?? null, update.heightSource ?? "", update.ownershipRecord?.parcelReference ?? "", update.ownershipRecord?.ulpinRecord ?? "", update.editorName, update.editNote]);
    ...
    if (update.ownershipRecord) {
      const ownership = await client.query<{ id: number }>(`
        ...
        RETURNING id
      `, [update.ownershipRecord.parcelReference, update.ownershipRecord.ulpinRecord, update.ownershipRecord.ownerName, update.ownershipRecord.ownershipBasis, update.ownershipRecord.rightsSummary ?? null, update.ownershipRecord.sourceReference ?? null, update.editorName]);
      await client.query(`
