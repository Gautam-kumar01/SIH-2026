...
## Ordered vertical-cadastre workspace upgrade

**Additional public reference coverage.** A second reference area, **Koramangala 5th Block**, is anchored to OpenStreetMap node `9790284747` at 12.9348429, 77.6189768. The import streamed Microsoft Global ML Building Footprints tile `123303312`, scanned 1,063,902 source features, and persisted 171 individually attributable footprint detections within 180 m of that reference point. The workspace search returned 80 matched footprints totaling 17,408.21 m² for the named public reference area. These are open-data detections, not parcel, campus, ownership, height, or other legal-cadastre claims. Sources: https://www.openstreetmap.org/node/9790284747 and https://github.com/microsoft/GlobalMLBuildingFootprints.

**Ordered review controls.** The dedicated Cesium workspace now exposes an interactive five-stage review sequence: **01 Source footprint**, **02 Building envelope**, **03 Floor & unit**, **04 Air & vertical rights**, and **05 Subsurface assets**. Only stage 01 is populated from live source geometry. Stages 02–05 make their evidence requirement visible and do not invent heights, plans, rights, utility depths, ownership, or legal ULPIN records.

**Interaction and responsiveness.** Desktop browser checks confirmed the Koramangala query focused 80 live source footprints, and selecting **Envelope** and **Floor / unit** updated the ordered inspector while retaining the “height required” and “plan required” constraints. The 390 × 844 viewport retained the ordered selector, map controls, source-count chip, stage labels, and vertical-profile inspector without obscuring controls.

**True-mobile interaction check.** The active browser measured 390 × 844 CSS pixels with the viewport meta tag enabled. In that viewport, the user-facing **Subsurface** ordered-review control changed the active stage to 05 / 05 and presented “Survey required”; the **Surface footprint references** layer was toggled from 4 to 3 active and restored to 4 active; and **Reset north** was used on the Cesium map. Throughout, the Koramangala source label, 80 matched footprint count, zero approved heights, no-inferred-ownership message, and evidence-warning panel remained visible.

**Operator account hit-target repair.** The Operator account control was moved above the passive CORS status in the sidebar footer, keeping its visible hit target clear at short browser heights. A normal semantic browser click on **Arjun Rao · Authority operator** opened the **Operator account** workspace; its administrator-access and immutable-source-geometry safeguards were visible.

**Home-control audit matrix completed.** The home navigation now has an individually exercised action path for: the live 3D workspace and volume links; parcel and ULPIN intelligence queries; data ingestion and processing queue; workspace settings; source footprint inspection and layer focus; audit and conflict workspaces; operator account; and all six evidence-resource controls. The Drone, LiDAR, GIS parcel layer, and Floor plan controls open the evidence-validation intake; GNSS/CORS, DEM/DSM, and conflict review route to the source-backed 3D workspace using the verified fallback reference when no search term is present. Each route presents an actionable workflow or map command rather than an inert placeholder.

## Three.js searched-building presentation

**Matched source footprint.** The Amity University Patna reference search returned 33 live PostGIS footprints. The structure inspector now renders the selected live footprint in a dedicated Three.js canvas from the same GeoJSON geometry, including its actual selected area (61.9 m² in the verified selection) and an interactive drag-to-rotate control. Where no approved height exists, the renderer displays a 1 m footprint plate and labels it “height awaiting authority approval”; it does not synthesize a tower, floors, or units.

**AI-assisted routing safeguard.** A server-side `resolveBuilding` route now first attempts a direct source match, then allows the built-in model to choose only from lexically eligible source-backed aliases. It will route Amity/Rupaspur wording to the existing Amity live geometry, but it rejects unrelated wording such as “Unknown tower in Patna.” The rejected query showed zero live footprints, “No verified 3D visual,” and the explicit Three.js unavailable state. Unit coverage also requires a lexical anchor, a catalog-supported alias, and model confidence of at least 0.6 before any AI-assisted focus is allowed.

**True-mobile Three.js verification.** A Chrome DevTools capture applied a 390 × 844 mobile device override to the live workspace and measured a 396 × 857 CSS viewport with a 338 × 186 px Three.js preview canvas. The captured Amity view showed 33 matched PostGIS footprints, the source-backed camera focus, the mobile ordered-review controls, the rendered source-footprint plate, the `MS-BUILDING-123133020-d0021a681d5903bd` identifier, height-awaiting-authority wording, and the no-inferred-ownership disclosure. The full mobile inspector remained single-column and readable without hiding the live source status.

**True-mobile matched/unmatched interaction verification.** In the same 396 × 857 mobile session, actual touch input entered **Amity building near Rupaspur** in the visible search field and pressed the visible **Locate** button. The workspace completed the guarded AI route to the Amity source area, reporting 33 live layers, 17,742.41 m², the source-backed heading, the safe routing notice, and the explicit status **“33 matched PostGIS footprints · camera focused on source geometry · EPSG:4326.”** A second touch-driven search for **Unknown tower in Patna** returned the explicit no-verified-3D heading, zero source footprints, the Three.js no-model state, and no live-geometry match notice.

**True-mobile Three.js touch verification.** A DevTools touch drag was applied directly to the mobile Three.js preview canvas between two captured states. Pixel comparison of the 341 × 189 px preview crop found 3,838 changed pixels of 64,449 (5.96%), confirming the rendered live source footprint rotated in response to the touch action while remaining a flat, authority-height-pending plate.

## Official Bihar cadastral candidate follow-up

**Candidate selection resolved.** The official Bihar BhuNaksha portal resolves the user-supplied “Danapur / Mauza 12” selection as **Patna → Danapur → Dinapur-Cum-Khagaul → 0344 Birbhan chak (12)**. The portal exposes RS and CS survey types, but neither a map sheet nor a Plot Info result was selected and no Khesra/plot number was supplied.

**Evidence boundary.** Birbhan chak (12) is therefore only a candidate revenue-Mauza context. It is not yet verified as Amity University Patna’s location or as a legal parcel, ULPIN, owner/holder record, cadastral boundary, approved height, or map geometry. The supplied aerial illustration was not used as geometry or evidence. The next admissible record is a BhuNaksha-selected plot screenshot or official export showing its Plot Info and map sheet, followed by the matching BiharBhumi/Jamabandi record.
